...
def select_cdd_nodes(args, graph_cdd_ids, adj_list, training_nodes_index, testing_nodes_index, subset):
    '''
    ...
    for gid in graph_cdd_ids:
        ...
        if subset == 'train':
            assert bkd_node_num <= len(training_nodes_index)
            cur_picked_nodes = rs.choice(training_nodes_index, bkd_node_num, replace=False)
        elif subset == 'test':
            assert bkd_node_num <= len(testing_nodes_index)
            cur_picked_nodes = rs.choice(testing_nodes_index, bkd_node_num, replace=False)
